...
# Depends on star_shared.py because plot-stars imports TRANSMISSIONS from it.
hail-mary.svg: plot-stars star_shared.py nearby.csv $(VENV)/bin/activate
	$(PYTHON) ./plot-stars

# Extended sky chart: all story stars including Wolf 359, Lalande 21185,
# Ross 128.  Covers RA 18–190°, Dec −37° to +44°.
hail-mary-sky.svg: plot-sky star_shared.py nearby.csv $(VENV)/bin/activate
	$(PYTHON) ./plot-sky

# Compact sky chart: only the five core story stars in Orion/Canis Major.
# Covers RA 18–142°, Dec −37° to +26°.
hail-mary-sky-limited.svg: plot-sky star_shared.py nearby.csv $(VENV)/bin/activate
